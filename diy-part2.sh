...
# 确保 backup-password 脚本存在并赋予密码文件权限设置为600，仅root可读
if [ -f "package/base-files/files/usr/bin/backup-password" ]; then
    echo "为 backup-password 脚本添加600权限..."
    chmod 600 package/base-files/files/usr/bin/backup-password
    # 检查权限是否设置成功
    ls -la package/base-files/files/usr/bin/backup-password
else
    echo "警告: 未找到 package/base-files/files/usr/bin/backup-password 文件，请检查路径是否正确。"
fi
